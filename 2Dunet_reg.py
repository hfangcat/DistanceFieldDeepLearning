...
from data2D_reg import load_train_data, load_test_data, preprocess_squeeze

K.set_image_data_format('channels_last')

project_name = '2D-Unet-reg'
img_rows = 512
img_cols = 512
img_depth = 1
smooth = 1.
...
def train():
    ...
    print(np.any(np.isnan(imgs_train)))
    print(np.any(np.isnan(imgs_mask_train)))
    imgs_mask_train = imgs_mask_train.astype('float32')
    imgs_train = imgs_train.astype('float32')
    # imgs_mask_train /= 32767.  # scale masks to [0, 1]
    # imgs_train /= 32767.  # scale masks to [0, 1]
    # imgs_mask_train = (imgs_mask_train - np.amin(imgs_mask_train)) / (np.amax(imgs_mask_train) - np.amin(imgs_mask_train))
...
def predict():
    ...
    imgs_mask_test = np.argmax(imgs_mask_test, axis=3)

    imgs_mask_test = imgs_mask_test[:, :, :, np.newaxis]
    ...
    for x in range(0, imgs_mask_test.shape[0]):
        imsave(os.path.join(pred_dir, 'pred_' + str(f"{count_processed:03}") + '.png'), imgs_mask_test[x] / 6)
        count_processed += 1
...
if __name__ == '__main__':
    img_mask_test = np.load('imgs_mask_test_reg.npy')
    img_mask_pred = np.load('test_mask_npy_dice/2D-Unet-reg_mask.npy')
    print(np.amax(img_mask_pred))
    print(np.amin(img_mask_pred))
    train()
    predict()