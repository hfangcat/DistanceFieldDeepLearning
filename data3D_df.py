...
def create_train_data():
    ...
    if argument == 1:
        ...
        np.save('imgs_train_3D_df.npy', imgs)
        np.save('imgs_mask_train_3D_df.npy', imgs_mask)
...
def load_train_data():
    imgs_train = np.load('imgs_train_3D_df.npy')
    imgs_mask_train = np.load('imgs_mask_train_3D_df.npy')
...
def create_test_data():
    ...
    if argument == 1:
        ...
        np.save('imgs_test_3D_df.npy', imgs)
        np.save('imgs_mask_test_3D_df.npy', imgs_mask)

        imgs = preprocess_squeeze(imgs)
        imgs_mask = preprocess_squeeze(imgs_mask)

        count_processed = 0
        pred_dir = 'test_preprocessed_3D_df'
        if not os.path.exists(pred_dir):
            os.mkdir(pred_dir)
        for x in range(0, imgs.shape[0]):
            for y in range(0, imgs.shape[1]):
                io.imsave(os.path.join(pred_dir, 'pre_processed_' + str(count_processed) + '.png'), imgs[x][y] / 6)
                count_processed += 1
                if (count_processed % 100) == 0:
                    print('Done: {0}/{1} test images'.format(count_processed, imgs.shape[0] * imgs.shape[1]))

        count_processed = 0
        pred_dir = 'test_mask_preprocessed_3D_df'
        if not os.path.exists(pred_dir):
            os.mkdir(pred_dir)
        for x in range(0, imgs_mask.shape[0]):
            for y in range(0, imgs_mask.shape[1]):
...
def load_test_data():
    imgs_test = np.load('imgs_test_3D_df.npy')
    return imgs_test
